...
def prepare_clips(hr_path, scale=4):
    ...
    file_names.sort(key=lambda x: int(x[:-4]))  # 将'.jpg'左边的字符转换成整数型进行排序
    lr_pils, hr_pils = [], []
...
def prepare_stereo(hr_path, hr_path2, scale=4):
    lr_pils, hr_pils = [], []
    hr_pil = Image.open(hr_path)
    hr_pil2 = Image.open(hr_path2)
    ####
    sizex, sizey = hr_pil.size
    hr_pil = hr_pil.resize((sizex - sizex % (16*scale), sizey - sizey % (16*scale)), Image.BICUBIC)
    hr_pil2 = hr_pil2.resize((sizex - sizex % (16*scale), sizey - sizey % (16*scale)), Image.BICUBIC)
    ####
    sizex, sizey = hr_pil.size
    hr_pil = hr_pil.crop((0, 0, sizex - sizex % scale, sizey - sizey % scale))
    hr_pil2 = hr_pil2.crop((0, 0, sizex - sizex % scale, sizey - sizey % scale))

    sizex, sizey = hr_pil.size
    lr_pil = hr_pil.resize((sizex // scale, sizey // scale), Image.BICUBIC)
    lr_pil2 = hr_pil2.resize((sizex // scale, sizey // scale), Image.BICUBIC)

    hr_pils.append(hr_pil)
    lr_pils.append(lr_pil)
    hr_pils.append(hr_pil2)
    lr_pils.append(lr_pil2)
    hr_coord = make_coord((sizey, sizex))
    cell = torch.ones_like(hr_coord)
    cell[:, :, 0] = cell[:, :, 0] * 2 / sizey
    cell[:, :, 1] = cell[:, :, 1] * 2 / sizex
    return lr_pils, hr_pils, hr_coord, cell
