...
import torch
import cv2
from lam.utils import _add_batch_one, _remove_batch
from lam.utils import grad_norm, IG_baseline, interpolation, isotropic_gaussian_kernel
...
def Path_gradient_continuous(numpy_image, coord, cell, model, attr_objective, path_interpolation_func, frame_index, cuda=False):
    """
    ...
    cv_numpy_image = np.concat([cv_numpy_image[0,:], cv_numpy_image[1,:]], axis=-1)
    ...
    for i in tqdm(range(image_interpolation.shape[0])):
        ...
        img_tensor.requires_grad_(True)
        if cuda:
            result = model(_add_batch_one(img_tensor).cuda(), _add_batch_one(coord).cuda(), _add_batch_one(cell).cuda())#.chunk(2, dim=1)
            target = attr_objective(result)
            result = result[:,:3,:,:]
            target.backward()
            ...
            result = model(_add_batch_one(img_tensor), _add_batch_one(coord), _add_batch_one(cell))#.chunk(2, dim=1)
            result = result[:,:3,:,:]
            target = attr_objective(result)
            target.backward()
            grad = img_tensor.grad.numpy()
...
def Path_gradient(numpy_image, model, attr_objective, path_interpolation_func, frame_index, cuda=False):
    ...
    for i in tqdm(range(image_interpolation.shape[0])):
        ...
        img_tensor.requires_grad_(True)
        if cuda:
            result = model(_add_batch_one(img_tensor).cuda())#.chunk(2, dim=1)
            result = result[:,:3,:,:]
            target = attr_objective(result)
            target.backward()
            grad = img_tensor.grad.cpu().numpy()
            ...
            result = model(_add_batch_one(img_tensor))#.chunk(2, dim=1)
            result = result[:,:3,:,:]
            target = attr_objective(result)
            target.backward()
            grad = img_tensor.grad.numpy()
