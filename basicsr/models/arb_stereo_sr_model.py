...
class ArbSSRModel(BaseModel):
    """StereoSRModel  for stereo image super-resolution."""

    def __init__(self, opt):
        super(ArbSSRModel, self).__init__(opt)
    ...
    def setup_optimizers(self):
        ...
        if train_opt.get('use_adapter_train',False):
            ...
            for name, param in self.net_g.named_parameters():
                ...
                if 'raft' in name or 'spynet' in name:
                    param.requires_grad = False
            

        # make lora layer trainable
        # lora.mark_lora_as_trainable(self.net_g, bias='lora_only')
    ...
    def check_gradients(self, model):
        ...
        for name, param in model.named_parameters():
            if param.grad is not None:
                ...
                logger.info(f'{name}: grad_mean={param.grad.mean():.6f}, '
                    f'grad_max={param.grad.max():.6f}, '
                    f'grad_min={param.grad.min():.6f}')
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_perceptual:
            l_percep, l_style = self.cri_perceptual(self.output[:,:3,:,:], self.gt[:,:3,:,:])
            l_percep2, l_style2 = self.cri_perceptual(self.output[:,3:,:,:], self.gt[:,3:,:,:])
            if l_percep is not None:
                l_percep = l_percep + l_percep2
                l_total += l_percep
                loss_dict['l_percep'] = l_percep
            if l_style is not None:
                l_style = l_style + l_style2
                l_total += l_style
        ...
        l_total.backward()
        self.check_gradients(self.net_g)
    ...
    def pre_process(self):
        ...
        if window_size[0]>0 and window_size[1] >0:
            ...
            self.coord = make_coord((self.scale*self.img.shape[-2], self.scale*self.img.shape[-1])).unsqueeze(0).to(self.device)
            self.cell = torch.ones_like(self.coord)
            self.cell[:, :, :, 0] = self.cell[:, :, :, 0] * 2 / self.img.shape[-2]/ self.scale
            self.cell[:, :, :, 1] = self.cell[:, :, :, 1] * 2 / self.img.shape[-2]/ self.scale
        else:
            self.img = self.lq
            self.mod_pad_w = 0
            self.mod_pad_h = 0
        ...
        if hasattr(self, 'net_g_ema'):
            ...
            with torch.no_grad():
                if self.img.size()[0] == 1:
                    self.output = self.net_g_ema(self.img, self.coord, self.cell)
                else:
                    for lq in self.img:
                        outs.append(self.net_g_ema(lq.unsqueeze(0)).cpu(), self.coord, self.cell)

        else:
            self.net_g.eval()
            with torch.no_grad():
                if self.img.size()[0] == 1:
                    self.output = self.net_g(self.img, self.coord, self.cell)
                else:
                    for lq in self.img:
                        outs.append(self.net_g(lq.unsqueeze(0)).cpu(), self.coord, self.cell)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if max_val is not None:
                if idx >= max_val:
                    continue
            img_name = osp.splitext(osp.basename(val_data['gt_path'][0]))[0].split('_')[0]
            ...
            if 'lq' in visuals:
                lq_imgs = tensor2img([visuals['lq'][:,:3],visuals['lq'][:,3:]],rgb2bgr=True)
            ...
            if save_img:
                ...
                # imwrite(lq_imgs[0], osp.join(self.opt['path']['visualization'], dataset_name,'GT', f'{img_name}_L.png'))
                # imwrite(lq_imgs[1], osp.join(self.opt['path']['visualization'], dataset_name,'GT',f'{img_name}_R.png'))
