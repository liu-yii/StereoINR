...
def stereo_single_random_crop_hw(img_gts, gt_patch_size_h, gt_patch_size_w):
    """Paired random crop.
    ...
    # randomly choose top and left coordinates for lq patch
    top = random.randint(0, h_gt - gt_patch_size_h)
    left = random.randint(0, w_gt - gt_patch_size_w)

    img_gts = [
        v[top:top + gt_patch_size_h, left:left + gt_patch_size_w, ...]
        for v in img_gts
    ]
    if len(img_gts) == 1:
        img_gts = img_gts[0]
...
def random_scale(imgs, gt_size,prob):

    '''

    :param imgs: image List
    :param gt_size: tuple (h,w)
    :return: resize list
    '''
    if np.random.random() > 1-prob:
        ...
        imgs = [cv2.resize(img,dsize=None,fx=scale,fy=scale,interpolation=cv2.INTER_LANCZOS4) for img in imgs]
        if scale_h < gt_h or scale_w < gt_w:
            imgs = [cv2.copyMakeBorder(img,0,max(gt_h-scale_h+1,0),0,max(gt_w-scale_w+1,0),borderType=cv2.BORDER_REFLECT) for img in imgs]
